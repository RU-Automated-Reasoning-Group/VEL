...
//    intervalNumPrecision = 300;
    ...
    ODE<Real> dynamics({"x4 - 0.25", "x5 + 0.25", "x6", "9.81 * sin(u1) / cos(u1)", "-9.81 * sin(u2) / cos(u2)", "u3 - 9.81", "0", "0", "0"}, vars);
...
//	setting.setTime(0.1);
  ...
  //  cout << "--------" << setting.g_setting.factorial_rec.size() << ", " << setting.g_setting.power_4.size() << ", " << setting.g_setting.double_factorial.size() << endl;
	 ...
	 */
	Interval init_x1(0.025, 0.05), init_x2(0.0, 0.025), init_x3(0.0, 0.0), init_x4(0.0, 0.0), init_x5(0.0, 0.0), init_x6(0.0, 0.0);
	...
	if (plot) {
		...
		plot_setting.plot_2D_interval_MATLAB("./outputs/", "qmpc1", result.tmv_flowpipes, setting);
	}
	cout << score << endl;
	return 0;
}