...
//    intervalNumPrecision = 300;
    ...
    ODE<Real> dynamics({"x3^3 - x2", "x3", "u", "0"}, vars);
...
//	setting.setTime(0.1);
  ...
  //  cout << "--------" << setting.g_setting.factorial_rec.size() << ", " << setting.g_setting.power_4.size() << ", " << setting.g_setting.double_factorial.size() << endl;
	 ...
	 */
	Interval init_x1(0.38, 0.4), init_x2(0.45, 0.47), init_x3(0.25, 0.27);
	...
	target_set.push_back(-0.4);
	target_set.push_back(-0.28);
	target_set.push_back(0.05);
	target_set.push_back(0.22);
	...
	if (plot) {
		...
		plot_setting.plot_2D_interval_MATLAB("./outputs/", "B5", result.tmv_flowpipes, setting);
	}
	cout << r_loss << endl;
	return 0;
}