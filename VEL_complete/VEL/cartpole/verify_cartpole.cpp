...
//	setting.setTime(0.1);
  ...
  //  cout << "--------" << setting.g_setting.factorial_rec.size() << ", " << setting.g_setting.power_4.size() << ", " << setting.g_setting.double_factorial.size() << endl;
	 ...
	 */
	Interval init_x1(-0.05, 0.05), init_x2(-0.05, 0.05), init_x3(atof(argv[8]), atof(argv[9])), init_x4(atof(argv[10]), atof(argv[11]));
	...
	for (int iter = 0; iter < 50; ++iter)
	{
        ...
        string strExpU = iteconstant(sig_str, s6, s7);
        // cout << strExpU << endl;
        ...
        dynamics.reach(result, initial_set, 0.02, setting, safeSet);
	...
	target_set.push_back(-2.4);
	target_set.push_back(2.4);
	target_set.push_back(-0.21);
	target_set.push_back(0.21);
	...
	if (plot) {
		...
		plot_setting.plot_2D_interval_MATLAB("./outputs/", "cartpole", result.tmv_flowpipes, setting);
	}
	cout << r_loss << endl;
	return 0;
}
