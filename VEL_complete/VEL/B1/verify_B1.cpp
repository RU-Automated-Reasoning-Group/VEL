...
//    intervalNumPrecision = 300;
    ...
    ODE<Real> dynamics({"x2", "u * x2 * x2 - x1", "0"}, vars);
...
//	setting.setTime(0.1);
  ...
  //  cout << "--------" << setting.g_setting.factorial_rec.size() << ", " << setting.g_setting.power_4.size() << ", " << setting.g_setting.double_factorial.size() << endl;
	 ...
	 */
	Interval init_x1(0.8, 0.9), init_x2(0.5, 0.6);
	...
	for (int iter = 0; iter < 35; ++iter)
	{
        ...
        dynamics.reach(result, initial_set, 0.2, setting, safeSet);
		...
		if (result.status == COMPLETED_SAFE || result.status == COMPLETED_UNSAFE || result.status == COMPLETED_UNKNOWN)
		{
			...
			if (inter_box[0].inf() < -1.5 || inter_box[0].sup() > 1.5 || inter_box[1].inf() < -1.5 || inter_box[1].sup() > 1.5) {
				cout << "unsafe" << endl;
			}
			// for (int i = 0; i < 3; ++i) {
	...
	if (plot) {
		...
		plot_setting.plot_2D_interval_MATLAB("./outputs/", "B1", result.tmv_flowpipes, setting);
	}
	cout << r_loss << endl;
	return 0;
}