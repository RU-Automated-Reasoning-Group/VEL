...
//    intervalNumPrecision = 300;
    ...
    ODE<Real> dynamics({"-x1 * (0.1 + (x1 + x2)^2)", "(u + x1) * (0.1 + (x1 + x2)^2)", "0"}, vars);
...
//	setting.setTime(0.1);
  ...
  //  cout << "--------" << setting.g_setting.factorial_rec.size() << ", " << setting.g_setting.power_4.size() << ", " << setting.g_setting.double_factorial.size() << endl;
	 ...
	 */
	Interval init_x1(0.8, 0.9), init_x2(0.4, 0.5);
	...
	if (plot) {
		...
		plot_setting.plot_2D_interval_MATLAB("./outputs/", "B3", result.tmv_flowpipes, setting);
	}
	cout << r_loss << endl;
	return 0;
}