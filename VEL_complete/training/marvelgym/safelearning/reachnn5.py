...
class GymReachNN5(gym.Env):
    ...
    def step(self,u):
        ...
        # new_x1 = x1 + (x1**3 - x2) * self.dt
        # new_x2 = x2 + (x3) * self.dt
        # new_x3 = x3 + (u) * self.dt
        # self.state = np.array([new_x1, new_x2, new_x3])

        center = np.array([-0.34, 0.135])
        ...
        # x1, x2, x3 = self.state
        # reward_near = 0
        # if x1 < -0.4 or x1 > -0.28 or x2 < 0.05 or x2 > 0.22:
        #     reward_near -= 1
...
class ReachNN5(GymWrapper):

    environment_name = 'ReachNN5'
    # what to set for threshold?
    reward_threshold = -3.75
    entry_point = "marvelgym.safelearning.reachnn5:GymReachNN5"
    ...
    def __init__(self, **kwargs):
        ...
        super(ReachNN5, self).__init__(config)
