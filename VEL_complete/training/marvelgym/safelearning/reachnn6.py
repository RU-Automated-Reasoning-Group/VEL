...
class GymReachNN6(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.image:
            ...
            self.high = np.array([2.0,  2.0, 2.0, 2.0])
            self.low = np.array([-2.0, -2.0, -2.0, -2.0])
    ...
    def f(self, state, t, u):
        x1, x2, x3, x4= state
        ff = np.array([x2, -x1 + 0.1 * math.sin(x3), x4, u])
        return ff
    ...
    def step(self,u):
        ...
        # new_x2 = x2 + (-x1 + 0.1 * math.sin(x3)) * self.dt
        # new_x3 = x3 + (x4) * self.dt
        # new_x4 = x4 + (u) * self.dt
        # self.state = np.array([new_x1, new_x2, new_x3, new_x4])

        x1, x2, x3, x4 = self.state
        reward_near = 0
        if x1 < -0.1 or x1 > 0.2 or x2 < -0.9 or x2 > -0.6:
            reward_near -= 1
        reward_safe = 0
        if x1 < -1.5 or x1 > 1.5 or x3 < -1.5 or x3 > 1.5:
            reward_safe -= 100
        # center = np.array([0.05, -0.75])
...
class ReachNN6(GymWrapper):

    environment_name = 'ReachNN6'
    # what to set for threshold?
    reward_threshold = -3.75
    entry_point = "marvelgym.safelearning.reachnn6:GymReachNN6"
    ...
    def __init__(self, **kwargs):
        ...
        super(ReachNN6, self).__init__(config)
